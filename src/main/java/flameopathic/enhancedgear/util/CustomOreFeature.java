...
public class CustomOreFeature extends Feature<CustomOreFeatureConfig> {
    public CustomOreFeature(Codec<CustomOreFeatureConfig> codec) {
        super(codec);
    }

    @Override
    public boolean generate(ServerWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator generator, Random random, BlockPos pos, CustomOreFeatureConfig config) {
    ...
    protected boolean generateVeinPart(WorldAccess world, Random random, CustomOreFeatureConfig config, double startX, double endX, double startZ, double endZ, double startY, double endY, int x, int y, int z, int size, int i) {
        int j = 0;
                                ...
                                for (int ak = ac; ak <= af; ++ak) {
                                    ...
                                    if (ah * ah + aj * aj + al * al < 1.0D) {
                                        ...
                                        if (!bitSet.get(am)) {
                                            ...
                                            if (config.target == world.getBlockState(mutable)) {
